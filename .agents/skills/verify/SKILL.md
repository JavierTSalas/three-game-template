...
Before driving, inventory the current PRD, visible controls, and every gameplay mode or
minigame. The steps below are the shipped-template minimum, not a fixed list: extend the
matrix for every added required action. Each mode must be finishable once through
touch/pointer input in a mobile-landscape viewport with no hardware keyboard and once through
its desktop keyboard and/or mouse path. A keyboard-only mode, an untested on-screen control,
or a required interaction that depends on hover/right-click is a verification failure.
